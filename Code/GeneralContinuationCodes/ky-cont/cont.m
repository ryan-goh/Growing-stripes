function [KDAT,PROF,Ind_prof,dxx,dyy,Lx,Ly] = cont(dtcont,cc,kx_init,ky_init,dir)


global mu0 mu c Lapu dy dx dxu xu xuext n Nx Ny N Xi Yi chi k0 z1 tang
...
N = Nx*Ny
tau=0;  %%%This was the quadratic coefficient (have to uncomment it
        %%%in the function files
plotbif = true;false;true;false; true;   %%Plots bifurcation diagrams
plotprof =  true;false;true;false;true;  %%Plots solution profiles
TIM =false;true;false; %time individual steps of continuation alg.
STEPS = 5000;
SAVE = true;true;true;  %%Save solution data to .mat file
save_int = 1;
...
if psuedoff
	lu = 2*pi; dxu = dy;
	xu = [dxu:dxu:2*pi]; n=1*length(y); n2 = Ny/2;
...
toc;
size(u)
%Interpolate the roll onto the full domain, and then cut it off
Mx = floor(Lx/2/pi);
...
u = getroll(sqrt(znew(end-1)^2+znew(end)^2));
...
%%%This is the full solution, in matrix form, FPROF will contain
%%%all solutions along bifurcation curve
FPROF = UI + VI;
...
KDAT = [KDAT;znew(N+1), znew(N+2)];%[znew(1:Nx)',znew(N+1), znew(N+2)];
PROF = [PROF,znew];
...
for j = 1:STEPS
	 ...
	 tic
      ...
      %%%if newton solver doesn't converge, reduce step size
      if exitflag < 1
          for iii=1:4
              dt = dt/5;
              iii;
      ...
      %%%Find Jacobian and tangent vector
      [FF,JJ]=shw(znew);tangn=JJ\em2;


      %%don't allow curve (kx,ky) to point in too big of an angle
      %%than in previous step
...
%      end
	...
	%disp('reduced')
      ...
      %%%Script to measure condition number
      ...
      z2 = znew;
      z1 = z1 + dt*tang;
      KDAT =[KDAT; znew(N+1), znew(N+2)];% [KDAT;znew(1:Nx)', znew(N+1), znew(N+2)];
      L2N = [L2N;norm(znew(1:N),2)];
      if TIM
          timer = toc
	end
      ...
      if plotbif
          % %%find any Eigenvalues and eigenvectors
 	         ...
 	         title(['k_x = ', num2str(z1(end-1)),' k_y = ' num2str(z1(end))])
      ...
      title(['\mu =',num2str(mu0),',  c =', num2str(c), ', ky = ', ...
             num2str(znew(end)),  ])
      xlabel('k_y')
      ylabel('k_x')

       subplot(1,2,2)
      plot(KDAT(:,end),sqrt(KDAT(:,end-1).^2+KDAT(:,end).^2))
      title(['|k| = ' num2str(sqrt(znew(end)^2+znew(end-1)^2))])
      xlabel('k_y')
      ylabel('|k|')
      drawnow
      ...
      drawnow
       ...
       if SAVE == true && mod(j,save_int)==0
                   PROF = [PROF,znew];
		   FPROF = [FPROF,UI+VI];
                   Ind_prof = [Ind_prof,jj+1];%length(KDAT(1,:))];


            fname = sprintf('ContData%d.mat',c);
            if kx_init ==0
                fname = sprintf('ContDataPerp%d.mat',c);
            end
           save(fname, 'c','mu0','PROF','FPROF','Ind_prof','x','y','mu','igL','igR','igW')
    ...
    if max(znew(floor(3.5*Nx/8):Nx:N))<0.05            %%%%track when front detaches
          LinfNorm=max(znew(floor(3.5*Nx/8):Ny:N))
          disp('spreading')
          break
    end

        if max(znew(floor(0.5*Nx/8):Nx:N))>0.05 %%%%detect boundary defects
